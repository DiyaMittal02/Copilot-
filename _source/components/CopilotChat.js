    ...
    try {
      ...
      setMessages(prev => {
        ...
        updated[updated.length - 1] = {
          role: 'ai',
          text: '⚠️ Error connecting to Gemini AI. Please check your API key and try again.',
          streaming: false
        };
        return updated;
      });
    ...
    <div className="chat-container">
      {!apiKey && (
        <div className="settings-panel">
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: '4px' }}>Gemini API Key Required</div>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '8px' }}>
              Get a free key at <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" style={{ color: '#818cf8' }}>aistudio.google.com</a>
            </div>
      ...
      {apiKey && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '8px' }}>
          <button className="btn btn-ghost" style={{ fontSize: '11px' }} onClick={() => { localStorage.removeItem('gemini_api_key'); setApiKey(''); }}>
            🔑 Change API Key
          </button>
        </div>
      )}
